...
import (
	...
	listers "lcostea.io/testcontroller/pkg/generated/listers/testcontroller/v1alpha1"
)
...
type Controller struct {
	...
	foosLister listers.GitSyncLister
...
func NewController(
	kubeclientset kubernetes.Interface,
	sampleclientset clientset.Interface,
	fooInformer informers.GitSyncInformer) *Controller {
	...
	controller := &Controller{
		...
		workqueue:       workqueue.NewNamedRateLimitingQueue(workqueue.DefaultControllerRateLimiter(), "GitSyncs"),
		recorder:        recorder,
	}
...
func (c *Controller) Run(threadiness int, stopCh <-chan struct{}) error {
	...
	klog.Info("Starting GitSync controller")
...
func (c *Controller) syncHandler(key string) error {
	...
	// Get the GitSync resource with this namespace/name
	gs, err := c.foosLister.GitSyncs(namespace).Get(name)
	if err != nil {
		// The GitSync resource may no longer exist, in which case we stop
		// processing.
		if errors.IsNotFound(err) {
			utilruntime.HandleError(fmt.Errorf("GitSync '%s' in work queue no longer exists", key))
			return nil
		}
